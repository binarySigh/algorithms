...
public class LC0174_CalculateMinimumHP {
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        int[][] dp = new int[m][n];
        int[][] hold = new int[m][n];
        dp[0][0] = dungeon[0][0] > 0 ? 0 : -(dungeon[0][0] - 1);
        hold[0][0] = dungeon[0][0] > 0 ? dungeon[0][0] : 1;
        //第一列
        for(int i = 1; i < m; i++){
            int need = dungeon[i][0] + hold[i - 1][0];
            if(need > 0){
                dp[i][0] = dp[i - 1][0];
                hold[i][0] = need;
            } else {
        ...
        //第一行
        for(int i = 1; i < n; i++){
            int need = dungeon[0][i] + hold[0][i - 1];
            if(need > 0){
                dp[0][i] = dp[0][i - 1];
                hold[0][i] = need;
            } else {
                dp[0][i] = dp[0][i - 1] - need + 1;
                hold[0][i] = 1;
            }
        }
        //普遍位置
