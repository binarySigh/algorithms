 ...
 *
 * 解答成功:
 * 		执行耗时:329 ms,击败了73.89% 的Java用户
 * 		内存消耗:94.3 MB,击败了100.00% 的Java用户
 *
 * @author binarySigh
 * @date 2021/6/20 16:56
 */
public class LC1600_ThroneInheritance {
    public Node king;
    public HashMap<String, Node> members;

    public LC1600_ThroneInheritance(String kingName) {
        this.king = new Node(kingName, true);
        this.members = new HashMap<>();
        this.members.put(kingName, this.king);
    }

    public void birth(String parentName, String childName) {
        Node newBirth = new Node(childName, true);
        Node cur = members.get(parentName);
        cur.birth(newBirth);
        this.members.put(childName, newBirth);
    }

    public void death(String name) {
        Node cur = members.get(name);
        cur.setDeath();
    }

    public List<String> getInheritanceOrder() {
        List<String> order = new ArrayList<>();
        getOrder(this.king, order);
        return order;
    }

    private void getOrder(Node cur, List<String> orders){
        if(cur.status){
            orders.add(cur.name);
        }
        for(int i = 0; i < cur.sons.size(); i++){
            Node curSon = cur.sons.get(i);
            getOrder(curSon, orders);
        }
    }
